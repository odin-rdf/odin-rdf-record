...
updated_at: 2026-08-19T17:21:08.492195+00:00
...
- [ ] Layout decided and recorded in RECORD-I-0001's Detailed Design: the
      log code's package placement (`record` vs a subpackage) and the CLI
      tool's future location, with the reason.
- [ ] Segment header (`log.md` §3): 64 bytes, big-endian, CRC-32C over
...
- [ ] Frame (§4): `u32 len` + CRC-32C over len‖body; `len == 0` and
      `len > MaxRecordSize` decode as *torn*, never as records; an unknown
      record kind is a typed failure, never a skip.
- [ ] CRC-32C is Castagnoli, confirmed against published test vectors;
      if `core:hash` lacks it, the table lives beside the framing code.
- [ ] Epoch commit (§5.1–5.3): term definitions carry the canonical
...
- [ ] Environment note (§5.5) carries the chain; segment seal (§5.4)
      encodes with `sigLen = 0` and stays outside the chain.
- [ ] Encode-side invariants refuse violations before bytes exist: epoch
      gaps, out-of-order term ids, inline IDs outside RECORD-A-0001's
      frozen range.
- [ ] Hash and CRC test vectors computed by an independent tool (Python
